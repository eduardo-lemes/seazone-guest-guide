...
export function AccessInfo({ operational }: AccessInfoProps) {
  return (
    <div className="space-y-3">
      <InfoCard icon={<Wifi size={16} />} title="Wi-Fi">
        <p className="text-sm text-slate-600">
          Rede: <span className="font-medium text-slate-800">{operational.wifiNetwork}</span>
        </p>
        <p className="text-sm text-slate-600">
          Senha: <span className="font-mono font-medium text-slate-800">{operational.wifiPassword}</span>
        </p>
      </InfoCard>

      <InfoCard icon={<Lock size={16} />} title="Acesso ao imóvel">
        <p className="text-sm text-slate-600">{operational.propertyAccessInstructions}</p>
        {operational.propertyPassword && (
          <p className="mt-1 text-sm text-slate-600">
            Código: <span className="font-mono font-medium text-slate-800">{operational.propertyPassword}</span>
          </p>
        )}
      </InfoCard>

      {operational.hasParkingSpot && (
        <InfoCard icon={<Car size={16} />} title="Estacionamento">
